...
def creat_pop(exploit_limit,fill_limit,rain_water,index_list,years,dim):
    ...
    for j in range(dim):
        ...
        for i in range(years):
            ...
            while np.any(sum_pop>targ) or np.all(sum_pop==0):
                for index in range(len(index_list[i])):
                    ...
                    pop[index_list[i][index], 3] = round(np.random.uniform(fill_limit[0], fill_limit[1]) / 800, 5)
...
def fun(pop,target_water_level,w,or_level):
    ...
    result=prediction-target_water_level
    out=[]
...
#补水量区间
exploit_limit=[[0,5000],[0,3000],[0,1000]]
#开采量区间
fill_limit=[0,12500]

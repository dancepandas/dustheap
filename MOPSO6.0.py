...
def func(tar_level,pop,test_data2,model_name='undergroundwater.keras'):
    model = keras.models.load_model(model_name)
    pre=[]
    pcr=[]
    for i in range(pop.shape[0]):
        y=model.predict([pop[i],test_data2])
        pre.append(np.sum(y,axis=1))
        pcr.append(y)
    pre_sum=np.sum(np.array(pre),axis=1)
    result=tar_level-pre_sum
...
def main(fill_limit,exploit_limit,dim,years,rain,tar_level,max_G,test_data2,model_name='undergroundwater.keras'):
    record=[]
    record_pop=[]
    tar=[]
    pop = creat_pop(fill_limit,exploit_limit,dim,years,rain)
    v = creat_v(fill_limit,exploit_limit,dim,years)
    fitness,pcr=func(tar_level,pop,test_data2,model_name)
    min_dis=min(fitness)
    ...
    for G in range(max_G):
        ...
        fitness1,pcr=func(tar_level,pop,test_data2,model_name)
...
max_G=5
#模型文件名称
model_name='undergroundwater.keras'
#上一年的逐月水位
test_data2=np.array([[[28.44 ],[28.30 ],[28.28],[27.64],[26.75],[26.25],[28.21],[30.85],[33.65],[35.16],[35.56],[35.79]]])

#主函数
pop,record=main(fill_limit,exploit_limit,dim,1,rain,tar_level,max_G,test_data2,model_name=model_name)
fit,lev=func(tar_level,np.array(pop),test_data2,model_name=model_name)
#输出数据
print('==============================================================================================')
print('正在输出数据…………')
path=r'输出.xlsx'
